...
def _startup():
    db.init_db()
    # Auto-import from jobs.json if DB is empty (e.g. fresh clone by Hamza)
...
def patch_tracking(body: TrackingUpdate):
    db.update_tracking(body.url, body.status, body.applied_date, body.notes)
    return {"ok": True}
...
.logo{width:28px;height:28px;border-radius:var(--r-sm);
  ...
  display:grid;place-items:center;font-size:14px;box-shadow:0 0 16px var(--acc-g);flex-shrink:0}
...
.btn{display:flex;align-items:center;gap:6px;padding:6px 14px;
  background:var(--s2);border:1px solid var(--b2);border-radius:var(--r-sm);
  font:inherit;font-size:12px;color:var(--t2);cursor:pointer;transition:120ms var(--ease)}
.btn:hover{background:var(--s3);color:var(--t1)}
.btn-primary{background:var(--acc-d);color:var(--acc);border-color:rgba(79,142,247,.3)}
.btn-primary:hover{background:rgba(79,142,247,.25);color:var(--t1)}
...
.fb{padding:8px 30px 8px 12px;background:var(--s2);border:1px solid var(--b1);
  ...
  background-repeat:no-repeat;background-position:right 9px center;transition:120ms var(--ease)}
...
select.ss{padding:4px 24px 4px 9px;border-radius:100px;font:inherit;font-size:11.5px;
  ...
  background-repeat:no-repeat;background-position:right 7px center;transition:120ms var(--ease)}
...
.tdot{width:7px;height:7px;border-radius:50%;background:var(--green);box-shadow:0 0 6px var(--green)}

.loading{display:flex;align-items:center;justify-content:center;height:200px;
  color:var(--t3);font-size:13px;gap:10px}
.spin{width:18px;height:18px;border:2px solid var(--b2);border-top-color:var(--acc);
...
<div class="toolbar">
  ...
  <select class="fb" id="ft" onchange="filter()">
    ...
    <option>IT</option><option>Security</option><option>Sales</option>
...
function exportCSV() {
  ...
  const lines = [['Title','Company','Location','Contract','Platform','Tags','URL','Status','Applied Date','Notes'].join(',')];
  rows.forEach(r => {
    const i = parseInt(r.dataset.i);
    const j = jobs[i];
    lines.push([j.title,j.company,j.location,j.contract,j.platform,
                (j.tags||[]).join('|'),j.url,j.status,j.applied_date,j.notes]
