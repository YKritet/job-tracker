...
_BASE = "https://www.cadremploi.fr"
_SEARCH = f"{_BASE}/emploi/liste_offres"
_HEADERS = {
...
class CadrEmploiSearcher(BaseSearcher):
    def search(self, keywords: str, location: str, count: int = 50) -> list[JobResult]:
        params = {"mots": keywords, "localisation": location, "rayon": "20"}
        try:
            resp = httpx.get(
        ...
        for card in cards[:count]:
            title_el = (
                ...
                or card.select_one("[class*='poste'] a")
                or card.select_one("a[href*='/emploi/annonce']")
            ...
            company_el = card.select_one(
                "[class*='company'], [class*='entreprise'], [class*='societe']"
            )
