...
_BASE = "https://www.keljob.com"
_SEARCH = f"{_BASE}/emploi/recherche"
_HEADERS = {
...
class KeljobSearcher(BaseSearcher):
    def search(self, keywords: str, location: str, count: int = 50) -> list[JobResult]:
        ...
        for card in cards[:count]:
            ...
            company_el = card.select_one(
                "[class*='company'], [class*='entreprise'], [class*='Company']"
            )
            company = company_el.get_text(strip=True) if company_el else ""

            loc_el = card.select_one(
                "[class*='location'], [class*='localisation'], [class*='lieu']"
            )
